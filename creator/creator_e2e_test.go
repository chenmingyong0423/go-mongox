...
func TestCreator_e2e_One(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			tc.before(tc.ctx, t)
			insertOneResult, err := creator.InsertOneOptions(tc.opts...).InsertOne(tc.ctx, tc.t)
			tc.after(tc.ctx, t)
...
func TestCreator_e2e_Many(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			tc.before(tc.ctx, t)
			insertManyResult, err := creator.InsertManyOptions(tc.opts...).InsertMany(tc.ctx, tc.t)
			tc.after(tc.ctx, t)
			if !tc.wantError(t, err) {
				return
			}
